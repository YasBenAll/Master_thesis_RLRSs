...
class ParetoArchive:
    ...
    def __init__(self, convex_hull: bool = False):
        ...
        self.catalog_coverage: List[np.ndarray] = []

    def add(self, candidate, evaluation: np.ndarray, catalog_coverage: np.ndarray):
        ...
        self.catalog_coverage.append(catalog_coverage)
        ...
        non_dominated_catalog_coverage = []

        for e, i, c in zip(self.evaluations, self.individuals, self.catalog_coverage):
            if tuple(e) in nd_candidates and tuple(e) not in non_dominated_evals_tuples:
                ...
                non_dominated_catalog_coverage.append(c)
        self.evaluations = non_dominated_evals
        self.individuals = non_dominated_individuals
        self.catalog_coverage = non_dominated_catalog_coverage
