...
class ItemEmbeddings(pl.LightningModule):
    ...
    def add_model_specific_args(parent_parser) -> MyParser:
        ...
        if '--item_embedd_dim' not in arguments:
            parser.add_argument('--item_embedd_dim', type=int, default = 10)
        return parser
    ...
    def from_pretrained(cls, checkpoint_path : str, device : torch.device):
        ...
        return cls(num_items, embedd_dim, weights = weights, device = "cuda")
    ...
    def get_from_env(cls, env, device, data_dir : str = None, embedd_path : str = None):
        ...
        return cls(num_items, embedd_dim, weights = embedd_weights,  device = "cuda")

    @classmethod
    def from_scratch(cls, num_items : int, embedd_dim : int, device : torch.device):
        return cls(num_items, embedd_dim,  device = "cuda")
...
class MFEmbeddings(ItemEmbeddings):
    ...
    def __init__(self, train_val_split_MF : float, batch_size_MF : int, lr_MF : float, num_neg_sample_MF : int,
                    weight_decay_MF : float, patience_MF : int, dataset_name: str, **kwargs) -> None:
    ...
    def collate_fn(self, batch : List[Tuple]) -> Dict:
        return {"user_ids" : torch.tensor([b[0] for b in batch], dtype = torch.long, device = "cuda"),
                "item_ids" : torch.tensor([b[1] for b in batch], dtype = torch.long, device = "cuda")}
    ...
    def train(self, dataset_path : str, data_dir : str) -> None:
        ...
        data = torch.load(dataset_path).observations
        users = {}
        sess_id = 0
        slates= []
        clickss = []
        for slate, clicks in tqdm(tzip(data["slate"], data["clicks"]), desc = "Preprocessing data"):
            slates.append(torch.tensor(slate))
            clickss.append(torch.tensor(clicks))
            if len(slates) == 100:
                sess = {"slate" : torch.stack(slates), "clicks" : torch.stack(clickss)}
                users[sess_id] = sess
                sess_id += 1
                slates= []
                clickss = []
        ...
        model = BPRMatrixFactorization(num_user, self.num_items, options, "cuda", "cuda")
        ...
        while True :
            ...
            for (n, batch) in enumerate(train_gen):
                if n % 1000 == 0:
                    ...
                    if val_loss < min_val_loss:
                        ...
                        import re
                        num_item = int(re.search(r'numitem(\d+)', self.dataset_name).group(1))
                        slate_size = int(re.search(r'slatesize(\d+)', self.dataset_name).group(1))
                        n_users = int(re.search(r'n_users(\d+)', self.dataset_name).group(1))


                        Path(data_dir).mkdir(parents=True, exist_ok=True)
                        torch.save(model.item_embeddings.weight.data, os.path.join(data_dir, f"item_embeddings_numitem{num_item}_slatesize{slate_size}_nusers{n_users}.pt"))
                        count = 0
                    else:
                        count += 1
            ...
            epoch += 1
        print("Finished training MF item embeddings.")
